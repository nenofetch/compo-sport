          ...
          <div class="col-12">
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        <form action="{{ route('gallery_images.update', $galleries->id) }}" method="POST"  class="form" enctype="multipart/form-data" data-parsley-validate>
                            @csrf
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <div class="row">
                                            <div class="col-md-2 mt-2 mb-2">
                                                @if ($galleries->image)
                                                    <img src="{{ asset('storage/gallery/' . $galleries->image) }}"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ $galleries->title }}" placeholder="Nama Foto" />
                                        @error('title')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                        ...
                                        <select name="gallery_categories_id" class="form-control @error('gallery_categories_id') is-invalid @enderror">
                                            ...
                                            @foreach ($gallery_categories as $row)
                                                <option id="categories_data" value="{{ $row->id }}"
                                                    {{ $galleries->gallery_categories_id == $row->id ? 'selected' : '' }}>
                                                    {{ $row->title }}
                                                </option>
                                            @endforeach
                                        </select>
