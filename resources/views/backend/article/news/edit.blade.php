...
@section('title', 'Ubah Data Artikel Berita')
          ...
          <div class="col-12">
            <div class="card">
                ...
                <div class="card-content">
                    <div class="card-body">
                        <form action="{{ route('articles_news.update', $articles_news->id) }}" method="POST"  class="form" enctype="multipart/form-data" data-parsley-validate>
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <input type="hidden" name="id" value="{{ $articles_news->id }}">
                                <div class="col-md-9 col-12">
                                    <div class="form-group">
                                        <label for="first-name-column">Judul</label>
                                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ $articles_news->title }}" placeholder="Judul" />
                                        @error('title')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="content" id="summernote" class="form-control @error('content') is-invalid @enderror">{{ $articles_news->content }}</textarea>
                                        @error('content')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6 mt-2 mb-2">
                                            @if ($articles_news->image)
                                                <img src="{{ asset('storage/article/' . $articles_news->image) }}"
                                        ...
                                        <select name="category_id" class="form-control @error('category_id') is-invalid @enderror">
                                            ...
                                            @foreach ($categories as $row)
                                                <option id="categories_data" value="{{ $row->id }}"
                                                    {{ $articles_news->category_id == $row->id ? 'selected' : '' }}>
                                                    {{ $row->title }}
                                                </option>
                                            @endforeach
                                        </select>
