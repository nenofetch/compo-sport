    ...
    <section class="section">
      <div class="card">
        <div class="card-header">
            <button class="btn btn-primary btn-sm" onclick="window.location='/pages_cms/create'"><i class="fas fa-plus"></i> Tambah Data</button>
        </div>
        <div class="card-body">
          <table class="table categories-table" id="table1">
            ...
            <tbody>
            @foreach ($pages_cms as $row)
              <tr>
                <input type="hidden" class="delete_id" value="{{ $row->id }}">
                <td>{{ $loop->iteration }}</td>
                <td>
                    @if ($row->image)
                        <img src="{{ asset('storage/pages/cms/' . $row->image) }}" width="100%" alt="image">
                    @else
                        Tidak memakai foto
                    @endif
                </td>
                ...
                <td>{{ $row->slug }}</td>
                <td>
                  <button class="btn btn-info btn-sm mb-2" onclick="window.location='/pages_cms/{{ $row->id }}'"><i class="fas fa-eye"></i> Detail</button>
                  <button class="btn btn-warning btn-sm mb-2" onclick="window.location='/pages_cms/{{ $row->id }}/edit'"><i class="fas fa-edit"></i> Edit</button>
